...
export function initCardsAnimation() {
  const cardConfigs = [
    { selector: ".tours__card", visibleClass: "tours__card--visible" },
    { selector: ".offers__card", visibleClass: "offers__card--visible" },
    { selector: ".tour-card", visibleClass: "is-visible" },
  ];

  const allCards = document.querySelectorAll(
    cardConfigs.map((c) => c.selector).join(", "),
  );
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          const config = cardConfigs.find((c) =>
  ...
  // (opacity:0 / translateX) ANTES de empezar a observar. Sin esto,
  // las cards que ya están visibles al cargar la página (sin necesidad
  // de scroll) se "teletransportan" en vez de animarse, porque el
  // navegador funde el estado inicial y final en un solo frame.
  ...
  window.addEventListener("scroll", () => {
    window.requestAnimationFrame(() => {
      ...
      if (
        ...
        ctaOriginalOffsetTop + cta.offsetHeight
      ) {
