import React, { useState } from "react";
import { supabase } from "../lib/supabaseClient";

export default function AuthModal({ isOpen, onClose }) {
  const [email, setEmail] = useState("");
  const [sent, setSent] = useState(false);
  const [loading, setLoading] = useState(false);
  const [errorMsg, setErrorMsg] = useState("");

  if (!isOpen) return null;

  // Login con Google
  const handleGoogleLogin = async () => {
    setErrorMsg("");
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/mis-reservas/`,
      },
    });
    if (error) setErrorMsg("Error al conectar con Google.");
  };

  // Login con Magic Link / OTP por Email
  const handleEmailLogin = async (e) => {
    e.preventDefault();
    setLoading(true);
    setErrorMsg("");

    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: {
        emailRedirectTo: `${window.location.origin}/panel-usuario/`,
      },
    });

    setLoading(false);
    if (error) {
      setErrorMsg("Error al enviar el enlace. Inténtalo de nuevo.");
    } else {
      setSent(true);
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-base-100 w-full max-w-md rounded-2xl p-6 shadow-2xl relative border border-base-200">
        {/* Botón Cerrar */}
        <button
          onClick={onClose}
          className="btn btn-sm btn-circle btn-ghost absolute right-4 top-4"
        >
          ✕
        </button>

        <h3 className="text-2xl font-bold text-center mb-2">Acceso al Panel</h3>
        <p className="text-sm text-center text-base-content/70 mb-6">
          Inicia sesión o regístrate para gestionar tus reservas
        </p>

        {errorMsg && (
          <div className="alert alert-error text-xs mb-4 p-2 rounded-lg">
            {errorMsg}
          </div>
        )}

        {/* 1. Botón Google */}
        <button
          onClick={handleGoogleLogin}
          type="button"
          className="btn btn-outline w-full rounded-xl flex items-center justify-center gap-2 border-base-300 hover:bg-base-200 hover:text-base-content mb-4"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
            />
          </svg>
          Continuar con Google
        </button>

        <div className="divider text-xs text-base-content/50 my-4">
          O CON TU EMAIL
        </div>

        {/* 2. Formulario Email */}
        {sent ? (
          <div className="text-center py-4 bg-primary/10 rounded-xl border border-primary/20">
            <i className="fa-solid fa-paper-plane text-2xl text-primary mb-2"></i>
            <p className="text-sm font-semibold">¡Enlace enviado!</p>
            <p className="text-xs text-base-content/70 mt-1">
              Revisa tu correo <span className="font-bold">{email}</span> para
              acceder directamente.
            </p>
          </div>
        ) : (
          <form onSubmit={handleEmailLogin} className="space-y-3">
            <input
              type="email"
              placeholder="tu@email.com"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              required
              className="input input-bordered w-full rounded-xl focus:outline-none focus:border-primary"
            />
            <button
              type="submit"
              disabled={loading}
              className="btn btn-primary w-full rounded-xl text-white"
            >
              {loading ? (
                <span className="loading loading-spinner"></span>
              ) : (
                "Recibir Enlace de Acceso"
              )}
            </button>
          </form>
        )}
      </div>
    </div>
  );
}
